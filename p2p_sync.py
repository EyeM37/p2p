...
class Node:
    def __init__(self, host, port): #กำหนดค่าเริ่มต้น
    ...
    def accept_connections(self): #รับการเชื่อมต่อใหม่จาก clients และสร้าง thread ใหม่เพื่อจัดการกับการเชื่อมต่อนั้น
        while True:
    ...
    def handle_client(self, client_socket):
        while True:
            try:
                ...
                self.process_message(message, client_socket) #เรียกฟังก์ชัน process_message เพื่อประมวลผลข้อความที่ได้รับจาก client
    ...
    def process_message(self, message, client_socket):
        ...
        elif message['type'] == 'sync_request': #ตรวจสอบประเภทของข้อความว่ามีค่าเป็น 'sync_request' หรือไม่
            self.send_all_transactions(client_socket) #เรียกฟังก์ชัน send_all_transactions เพื่อส่งข้อมูล transactions ทั้งหมดไปยัง client ที่ร้องขอการซิงค์
        elif message['type'] == 'sync_response': #ตรวจสอบประเภทของข้อความว่ามีค่าเป็น 'sync_response' หรือไม่
            self.receive_sync_data(message['data']) #เรียกฟังก์ชัน receive_sync_data เพื่อรับและประมวลผลข้อมูล transactions ที่ได้รับจากการซิงค์   
        else:
            print(f"Received message: {message}") #หากไม่ใช่ทั้งหมดข้างต้นให้แสดงข้อความว่ามีการรับข้อความ พร้อมกับข้อความนั้น

    def add_transaction(self, transaction):
        # เพิ่ม transaction ใหม่และบันทึกลงไฟล์
        if transaction not in self.transactions: #ตรวจสอบว่า transaction ยังไม่ได้ถูกเพิ่มเข้าไป
    ...
    def request_sync(self, peer_socket):
        ...
        sync_request = json.dumps({"type": "sync_request"}).encode('utf-8') # สร้างคำขอซิงโครไนซ์ แปลงข้อความ JSON เป็นบิต (encode เป็น 'utf-8') และเก็บในตัวแปร sync_request
        peer_socket.send(sync_request) #ส่งข้อความคำขอซิงโครไนซ์ไปยัง peer ที่เชื่อมต่ออยู่

    def send_all_transactions(self, client_socket):
        # ส่ง transactions ทั้งหมดไปยังโหนดที่ขอซิงโครไนซ์
        sync_data = json.dumps({
            "type": "sync_response", # ประเภทของข้อความเป็นการตอบสนองการซิงโครไนซ์
            "data": self.transactions # ข้อมูลธุรกรรมทั้งหมดของโหนดปัจจุบัน
        }).encode('utf-8') #แปลงข้อความ JSON เป็นบิต
        client_socket.send(sync_data) # ส่งข้อความซิงโครไนซ์ไปยังโหนดที่ขอซิงโครไนซ์
    ...
    def receive_sync_data(self, sync_transactions):
        ...
        for tx in sync_transactions: 
            self.add_transaction(tx) # วนลูปผ่านข้อมูลธุรกรรมที่ได้รับและเพิ่มลงในรายการธุรกรรมของโหนดปัจจุบัน
        print(f"Synchronized {len(sync_transactions)} transactions.") # แสดงข้อความว่ามีการซิงโครไนซ์ธุรกรรมจำนวนเท่าใด

if __name__ == "__main__":
    if len(sys.argv) != 2: # ตรวจสอบว่ามีการส่งอาร์กิวเมนต์พอร์ตผ่านบรรทัดคำสั่งมาหรือไม่
        print("Usage: python script.py <port>") # แสดงข้อความการใช้งานที่ถูกต้อง
